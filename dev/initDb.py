import sqlparse
import pymysql
from applications.config.database import HOST, USERNAME, PASSWORD, DATABASE


def is_exist_database():
    db = pymysql.connect(host=HOST, user=USERNAME, password=PASSWORD, charset='utf8mb4')
    cursor1 = db.cursor()
...
def init_db():
    ...
    print('表创建成功')
    print('欢迎使用pear-admin-flask,请使用 falsk run 命令启动程序')
